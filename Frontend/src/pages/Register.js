          ...
          <div className="border-0 rounded-4 p-0" style={{ boxShadow: isModal ? 'none' : '0 4px 32px #bae6fd33', background: 'linear-gradient(135deg, #f0f7ff 0%, #fff 100%)' }}>
            <div className="p-0">
              <div className="text-center mb-3 pt-2">
                ...
                <h2 className="fw-bold mb-1" style={{ fontSize: '2.1rem', color: '#0ea5e9', letterSpacing: 0.2 }}>Create Your Account</h2>
                <p className="text-muted mb-0" style={{ fontSize: '1.08rem' }}>Sign up in seconds and start your journey</p>
              </div>
              <hr style={{ border: 0, borderTop: '1.5px solid #e5e7eb', margin: '0 0 18px 0' }} />
              {error && <Alert variant="danger" className="rounded-3">{error}</Alert>}
              <div className="d-flex flex-column align-items-center gap-2 mb-3">
                <Form.Group className="mb-2 w-100">
                  <Form.Label className="fw-semibold mb-1">Sign up as</Form.Label>
                  <Form.Select
                ...
                <div style={{ width: 240 }}>
                  <div className="google-btn-hover">
                    <GoogleLogin onSuccess={handleGoogleSuccess} onError={handleGoogleError} width="100%" text="signup_with" shape="rectangular" size="large" />
                  </div>
                  <div className="text-center mt-2" style={{ fontWeight: 500, color: '#0ea5e9', fontSize: '1.08rem', letterSpacing: 0.1 }}>Sign up with Google</div>
                </div>
              </div>
              <div className="text-center mt-3 pb-2">
                <span className="text-muted">Already have an account?</span>
                <Button variant="link" className="fw-semibold ps-2" style={{ color: '#0ea5e9', fontSize: '1.08rem', textDecoration: 'underline' }} onClick={() => window.dispatchEvent(new CustomEvent('showLoginModal'))}>
                  Login
                </Button>
              </div>
            </div>
